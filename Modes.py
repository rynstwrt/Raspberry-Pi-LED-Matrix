...
def dual_stripes(color1, color2):
    blank()
    while True:
        if current_function is not None and current_function != dual_stripes:
            return
...
def temperature(city, state, country, api_key, units):
    while True:
        ...
        response = requests.get(
            "http://api.openweathermap.org/data/2.5/weather?q={},{},{}&appid={}&units={}".format(city, state, country,
                                                                                                 api_key, units))
        ...
        draw.text((ceil(NUM_COLS / 2 - width / 2), ceil(NUM_ROWS / 2 - height / 2) - 1), temperature_text, color,
                  font=font)
...
def rainbow_equalizer():
    ...
    while True:
        ...
        for x in range(len(y_values)):
            ...
            for y in range(NUM_ROWS):
                pixels[get_pixel_index_at(x, NUM_ROWS - y - 1)] = wheel(
                    ((x + y) * (255 // NUM_ROWS)) & 255) if y <= value else COLOR_BLACK
